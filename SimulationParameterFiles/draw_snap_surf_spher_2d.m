...
% dir_coord='./input_stfa1.5t4.5';  
% dir_metric='./input_stfa1.5t4.5'; 
% dir_media='./input_stfa1.5t4.5'; 
% dir_source='./input_stfa1.5t4.5'; 
% dir_station='./input_stfa1.5t4.5';
% dir_out='./output_stfa1.5t4.5';
dir_coord='./input';  
dir_metric='./input'; 
dir_media='./input'; 
dir_source='./input'; 
...
%e-w crosssection
n1=20;n2=300;dn=2;% saved every 4 simulation time steps
...
%imagesc(topoz); drawnow; pause; hold on;
...
%             squeeze(permute(v,[2 1 3])));
   pcolor(squeeze(permute(y,[2 1 3])), ...
            squeeze(permute(x,[2 1 3])), ...
            squeeze(permute(v,[2 1 3])));
else
...
%colorbar('location','manual','position',[0.9 0.1 0.03 0.2])
...
%axis tight
%axis off
%axis([min(y(1,:))-1.0e5 max(y(1,:))+5.0e5 min(x(:,1))-5.0e5 max(x(:,1))+1.0e5 min(min(z)) max(max(z))]);
